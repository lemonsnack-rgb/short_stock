...
UNIVERSE_HEADERS = [
    ...
    "atr","ema","score","in_atr_buy","in_pivot_buy","target_3pct"
]
TOP10_HEADERS = [
    "rank","ticker","name","close",
    "buy_atr","sell_atr","buy_pivot","sell_pivot","stop","score","target_3pct"
]
...
def calc_levels(tkr: str, ref_date: date):
    """전일 H/L/C 기반 Pivot, 20일 EMA/ATR 기반 밴드 + 간단 스코어 + 3% 목표가"""
    ymd = ref_date.strftime("%Y%m%d")
    ...
    # 3% 목표가
    target_price = int(round(c * (1.0 + DAY_TARGET_PCT/100.0)))
...
def write_universe_and_top10(rows: list, ref: date, news: dict, name_map: dict[str, str], reasons: dict[str, list[str]]):
    ...
    out = top[["ticker","name","close","buy_atr","sell_atr","buy_pivot","sell_pivot","stop","score","target_3pct"]].copy()
